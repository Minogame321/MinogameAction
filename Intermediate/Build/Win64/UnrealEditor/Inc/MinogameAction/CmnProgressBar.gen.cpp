// Copyright Epic Games, Inc. All Rights Reserved.
/*===========================================================================
	Generated code exported from UnrealHeaderTool.
	DO NOT modify this manually! Edit the corresponding .h files instead!
===========================================================================*/

#include "UObject/GeneratedCppIncludes.h"
#include "MinogameAction/CmnProgressBar.h"
PRAGMA_DISABLE_DEPRECATION_WARNINGS
void EmptyLinkFunctionForGeneratedCodeCmnProgressBar() {}
// Cross Module References
	MINOGAMEACTION_API UClass* Z_Construct_UClass_UCmnProgressBar_NoRegister();
	MINOGAMEACTION_API UClass* Z_Construct_UClass_UCmnProgressBar();
	UMG_API UClass* Z_Construct_UClass_UUserWidget();
	UPackage* Z_Construct_UPackage__Script_MinogameAction();
	UMG_API UClass* Z_Construct_UClass_UProgressBar_NoRegister();
// End Cross Module References
	void UCmnProgressBar::StaticRegisterNativesUCmnProgressBar()
	{
	}
	IMPLEMENT_CLASS_NO_AUTO_REGISTRATION(UCmnProgressBar);
	UClass* Z_Construct_UClass_UCmnProgressBar_NoRegister()
	{
		return UCmnProgressBar::StaticClass();
	}
	struct Z_Construct_UClass_UCmnProgressBar_Statics
	{
		static UObject* (*const DependentSingletons[])();
#if WITH_METADATA
		static const UECodeGen_Private::FMetaDataPairParam Class_MetaDataParams[];
#endif
#if WITH_METADATA
		static const UECodeGen_Private::FMetaDataPairParam NewProp_m_pProgressBar_MetaData[];
#endif
		static const UECodeGen_Private::FObjectPropertyParams NewProp_m_pProgressBar;
		static const UECodeGen_Private::FPropertyParamsBase* const PropPointers[];
		static const FCppClassTypeInfoStatic StaticCppClassTypeInfo;
		static const UECodeGen_Private::FClassParams ClassParams;
	};
	UObject* (*const Z_Construct_UClass_UCmnProgressBar_Statics::DependentSingletons[])() = {
		(UObject* (*)())Z_Construct_UClass_UUserWidget,
		(UObject* (*)())Z_Construct_UPackage__Script_MinogameAction,
	};
#if WITH_METADATA
	const UECodeGen_Private::FMetaDataPairParam Z_Construct_UClass_UCmnProgressBar_Statics::Class_MetaDataParams[] = {
		{ "Comment", "/**\n * \n */" },
		{ "IncludePath", "CmnProgressBar.h" },
		{ "ModuleRelativePath", "CmnProgressBar.h" },
	};
#endif
#if WITH_METADATA
	const UECodeGen_Private::FMetaDataPairParam Z_Construct_UClass_UCmnProgressBar_Statics::NewProp_m_pProgressBar_MetaData[] = {
		{ "BindWidget", "" },
		{ "EditInline", "true" },
		{ "ModuleRelativePath", "CmnProgressBar.h" },
	};
#endif
	const UECodeGen_Private::FObjectPropertyParams Z_Construct_UClass_UCmnProgressBar_Statics::NewProp_m_pProgressBar = { "m_pProgressBar", nullptr, (EPropertyFlags)0x0040000000080008, UECodeGen_Private::EPropertyGenFlags::Object, RF_Public|RF_Transient|RF_MarkAsNative, 1, STRUCT_OFFSET(UCmnProgressBar, m_pProgressBar), Z_Construct_UClass_UProgressBar_NoRegister, METADATA_PARAMS(Z_Construct_UClass_UCmnProgressBar_Statics::NewProp_m_pProgressBar_MetaData, UE_ARRAY_COUNT(Z_Construct_UClass_UCmnProgressBar_Statics::NewProp_m_pProgressBar_MetaData)) };
	const UECodeGen_Private::FPropertyParamsBase* const Z_Construct_UClass_UCmnProgressBar_Statics::PropPointers[] = {
		(const UECodeGen_Private::FPropertyParamsBase*)&Z_Construct_UClass_UCmnProgressBar_Statics::NewProp_m_pProgressBar,
	};
	const FCppClassTypeInfoStatic Z_Construct_UClass_UCmnProgressBar_Statics::StaticCppClassTypeInfo = {
		TCppClassTypeTraits<UCmnProgressBar>::IsAbstract,
	};
	const UECodeGen_Private::FClassParams Z_Construct_UClass_UCmnProgressBar_Statics::ClassParams = {
		&UCmnProgressBar::StaticClass,
		nullptr,
		&StaticCppClassTypeInfo,
		DependentSingletons,
		nullptr,
		Z_Construct_UClass_UCmnProgressBar_Statics::PropPointers,
		nullptr,
		UE_ARRAY_COUNT(DependentSingletons),
		0,
		UE_ARRAY_COUNT(Z_Construct_UClass_UCmnProgressBar_Statics::PropPointers),
		0,
		0x00B010A0u,
		METADATA_PARAMS(Z_Construct_UClass_UCmnProgressBar_Statics::Class_MetaDataParams, UE_ARRAY_COUNT(Z_Construct_UClass_UCmnProgressBar_Statics::Class_MetaDataParams))
	};
	UClass* Z_Construct_UClass_UCmnProgressBar()
	{
		if (!Z_Registration_Info_UClass_UCmnProgressBar.OuterSingleton)
		{
			UECodeGen_Private::ConstructUClass(Z_Registration_Info_UClass_UCmnProgressBar.OuterSingleton, Z_Construct_UClass_UCmnProgressBar_Statics::ClassParams);
		}
		return Z_Registration_Info_UClass_UCmnProgressBar.OuterSingleton;
	}
	template<> MINOGAMEACTION_API UClass* StaticClass<UCmnProgressBar>()
	{
		return UCmnProgressBar::StaticClass();
	}
	DEFINE_VTABLE_PTR_HELPER_CTOR(UCmnProgressBar);
	struct Z_CompiledInDeferFile_FID_MinogameAction_Source_MinogameAction_CmnProgressBar_h_Statics
	{
		static const FClassRegisterCompiledInInfo ClassInfo[];
	};
	const FClassRegisterCompiledInInfo Z_CompiledInDeferFile_FID_MinogameAction_Source_MinogameAction_CmnProgressBar_h_Statics::ClassInfo[] = {
		{ Z_Construct_UClass_UCmnProgressBar, UCmnProgressBar::StaticClass, TEXT("UCmnProgressBar"), &Z_Registration_Info_UClass_UCmnProgressBar, CONSTRUCT_RELOAD_VERSION_INFO(FClassReloadVersionInfo, sizeof(UCmnProgressBar), 2115740053U) },
	};
	static FRegisterCompiledInInfo Z_CompiledInDeferFile_FID_MinogameAction_Source_MinogameAction_CmnProgressBar_h_2445745684(TEXT("/Script/MinogameAction"),
		Z_CompiledInDeferFile_FID_MinogameAction_Source_MinogameAction_CmnProgressBar_h_Statics::ClassInfo, UE_ARRAY_COUNT(Z_CompiledInDeferFile_FID_MinogameAction_Source_MinogameAction_CmnProgressBar_h_Statics::ClassInfo),
		nullptr, 0,
		nullptr, 0);
PRAGMA_ENABLE_DEPRECATION_WARNINGS
